...
echo "This script has been replaced by 'lift' since July 2025 and is only here for future reference."
exit 1

# docker network create -d bridge punctual
net=punctual
...
if target fredericocraft; then
	docker run -dit --name fredericocraft \
		-p 25565:25565/tcp \
		-v /volume1/docker/fredericocraft:/data:rw \
		-e 'TYPE=paper' \
		-e 'VERSION=1.16.5' \
		-e 'EULA=true' \
		-e 'UID=1000' \
...
if target vik; then
	...
	docker run -dit --name vik \
		...
		-e "WEBHOOK_URL=http://localhost:4000/api/webhooked" \
		-e "KV_ROOT=/kv" \
		-e 'AUTH_PASSWORD=REDACTED' \
