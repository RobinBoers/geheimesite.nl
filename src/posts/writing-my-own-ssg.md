...
How does one build a SSG, you might ask? Well, it's actually fairly simple, because a lot of the building blocks are already out there. For my project, I chose the [Liquid templating language](//shopify.github.io/liquid/). Luckily, there's already an amazing Javascript library for using Liquid out there, called [LiquidJS](//liquidjs.com). The same goes for [YAML parsing](https://www.npmjs.com/package/js-yaml) and Markdown (using [gemdown](//github.com/audiodude/gemdown) and [marked](//marked.js.org) respectively).
