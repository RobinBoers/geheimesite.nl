...
echo "Building CSS..."
npx tailwindcss -c tailwind.config.js -i src/assets/css/source.css -o src/assets/css/main.css --minify > /dev/null 2>&1 || exit 1

if ! $VALIDATE; then
    echo "Building site..."
    HUGO_UGLYURLS=true hugo --quiet --ignoreCache || exit 1
else
