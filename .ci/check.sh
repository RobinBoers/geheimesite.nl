...
FILES=$(fd --glob "*.html" --ignore-file ../.checkignore --strip-cwd-prefix)
USER_AGENT="Mozilla/5.0 (X11; Linux x86_64; rv:60.0) Gecko/20100101 Firefox/81.0"

echo "Validating HTML..."
html5validator $FILES --ignore 'CSS' 'style' || FAIL=1

echo "Audit site for build errors..."
grep -inorE "raw HTML omitted|ZgotmplZ|\[i18n\]|\(<nil>\)|(&lt;nil&gt;)|hahahugo" . && FAIL=3
...
do
    ...
    do
        echo "$URL" | grep -E -f .checkignore > /dev/null 2>&1 && continue
        case "$URL" in
            https://*|http://*)
